...
UserSchema.methods.addSession = function(sessionData: Partial<IUserSession>): void {
  ...
  if (session.deviceId) {
    this.sessions = this.sessions.filter((s: IUserSession) => s.deviceId !== session.deviceId);
  }
...
UserSchema.methods.removeSession = function(deviceId: string): void {
  this.sessions = this.sessions.filter((s: IUserSession) => s.deviceId !== deviceId);
};
...
UserSchema.methods.canAccessClinic = function(clinicName: string): boolean {
  if (this.permissions.canAccessAllClinics) return true;
...
UserSchema.methods.getDefaultPermissions = function(role: UserRole): IUserPermissions {
