...
const DISCONTINUED_PRODUCT_CODES = [
  'AC', 'AC50', 'AC60', 'AC80', 'ALLE', 'ANX', 'ARTH', 'BNP', 'BPH', 
  'CANPREV5HTP', 'CANPREVALA', 'CANPREVEP', 'CANPREVHH', 'CANPREVKRILL',
  'CANPREVMAG', 'CANPREVMVIT', 'CANPREVOM3', 'CANPREVPRB', 'CANPREVVITB',
  'CANPREVVITC', 'CANPREVVITD', 'CHLST', 'CIRCUL', 'CX2', 'DETOX',
  'DIGT', 'ED', 'EMOT', 'ENRG', 'ENZM', 'EUCAN', 'EUHRT', 'EUJNT',
  'EUMEM', 'EUMENS', 'EUSTOM', 'EUWOM', 'FIBR', 'FLSHES', 'GLANDM',
  'GLANDS', 'HBP', 'HEADNO', 'HLTH', 'HMN', 'HRBN', 'IMUN', 'INFLAM',
  'INSOM', 'LIV', 'METAB', 'MST', 'NAIL', 'NUTRA', 'OBES', 'OSTEO',
  'PARA', 'PMS', 'RESP', 'SKIN', 'SMKG', 'STRESS', 'SUP', 'THRYD',
  'UTI', 'VIT', 'WGT'
];
...
ProductSchema.virtual('popularityScore').get(function(this: IProduct) {
  ...
  return Math.round((totalOrders * 0.4 + (totalRevenue / 100) * 0.4 + recencyFactor * 0.2) * 100) / 100;
});

// Pre-save middleware for business rules
ProductSchema.pre<IProduct>('save', async function(next) {
  // Auto-set discontinued status based on business rules
  if (DISCONTINUED_PRODUCT_CODES.includes(this.productKey.toString()) || 
  ...
  if (this.isNew && (!this.identifier || this.identifier.length === 0)) {
    this.identifier = [{
      system: 'http://bala-visio.com/fhir/product-identifier',
  ...
  if (this.isNew && (!this.code || this.code.length === 0)) {
    this.code = [{
      system: 'http://bala-visio.com/fhir/product-codes',
...
// Instance Methods
ProductSchema.methods.updateUsageStats = async function(orderValue: number, orderDate?: Date): Promise<void> {
  this.usage.totalOrders += 1;
  this.usage.totalRevenue += orderValue;
  this.usage.lastOrderDate = orderDate || new Date();
  this.usage.averageOrderValue = this.usage.totalRevenue / this.usage.totalOrders;
  this.usage.popularityScore = this.popularityScore;
  return this.save();
};

ProductSchema.methods.isAvailableForClinic = function(clinicName: string): boolean {
  if (this.isDiscontinued || !this.isActive) return false;
  if (this.isAvailableForAllClinics) return true;
...
ProductSchema.methods.toFHIRResource = function(): any {
  ...
  if (this.resourceType === FHIRProductType.SERVICE) {
    return {
      ...
      category: [{
        coding: [{
          system: 'http://bala-visio.com/fhir/service-categories',
          code: this.category,
...
ProductSchema.statics.findActiveByClinic = function(clinicName: string, options: any = {}) {
  const filter = {
    isActive: true,
...
ProductSchema.statics.findByCategory = function(category: ProductCategory, options: any = {}) {
  const filter = {
    category,
    isActive: true,
    isDiscontinued: false,
...
ProductSchema.statics.findNonDiscontinued = function(options: any = {}) {
  const filter = {
    isDiscontinued: false,
    productKey: { $nin: DISCONTINUED_PRODUCT_CODES.map(code => parseInt(code) || code) },
    ...options.filter
  };
